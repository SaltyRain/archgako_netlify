...
exports.getPathDepth = exports.getAllResponses = exports.parseLinkHeader = exports.responseParser = exports.parseResponse = exports.filterByPropExtension = void 0;
...
const filterByPropExtension = (extension, propName) => arr => arr.filter(el => (0, _get2.default)(el, propName, '').endsWith(extension.startsWith('.') ? extension : ".".concat(extension)));
...
const parseResponse = async (res, _ref2) => {
  let {
    expectingOk = true,
    format = 'text',
    apiName = ''
...
const parseLinkHeader = (0, _flow2.default)([linksString => linksString.split(','), (0, _map2.default)(str => str.trim().split(';')), (0, _map2.default)((_ref3) => {
  let [linkStr, keyStr] = _ref3;
  return [keyStr.match(/rel="(.*?)"/)[1], linkStr.trim().match(/<(.*?)>/)[1].replace(/\+/g, '%20')];
}), _fromPairs2.default]);
exports.parseLinkHeader = parseLinkHeader;

const getAllResponses = async function getAllResponses(url) {
  let options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  let linkHeaderRelName = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'next';
  ...
  while (req && responseCount < maxResponses) {
    ...
    req = nextURL && _unsentRequest.default.fromFetchArguments(nextURL, {
      headers
    });
