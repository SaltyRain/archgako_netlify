...
const getLargeMediaPatternsFromGitAttributesFile = (0, _flow2.default)([parseGitAttributesFileToPatternAttributePairs, (0, _filter2.default)( // eslint-disable-next-line @typescript-eslint/no-unused-vars
(_ref4) => {
  let [_pattern, attributes] = _ref4;
  return attributes.filter === 'lfs' && attributes.diff === 'lfs' && attributes.merge === 'lfs';
}), (0, _map2.default)((_ref5) => {
  let [pattern] = _ref5;
  return pattern;
})]);
exports.getLargeMediaPatternsFromGitAttributesFile = getLargeMediaPatternsFromGitAttributesFile;

const matchPath = (_ref6, path) => {
  let {
    patterns
  } = _ref6;
...
const resourceExists = async (_ref7, _ref8) => {
  let {
    rootURL,
...
const getTransofrmationsParams = t => "?nf_resize=".concat(t.nf_resize, "&w=").concat(t.w, "&h=").concat(t.h);

const getDownloadURL = (_ref9, _ref10) => {
  let {
    rootURL,
    transformImages: t,
    makeAuthorizedRequest
  ...
  return makeAuthorizedRequest("".concat(rootURL, "/origin/").concat(sha).concat(t && Object.keys(t).length > 0 ? getTransofrmationsParams(t) : '')).then(res => res.ok ? res : Promise.reject(res)).then(res => res.blob()).then(blob => URL.createObjectURL(blob)).catch(err => {
    console.error(err);
    return Promise.resolve('');
  });
};

const getResourceDownloadURLArgs = (_clientConfig, objects) => {
  const result = objects.map((_ref11) => {
    let {
      sha
    } = _ref11;
    return [sha, {
...
const uploadOperation = objects => ({
  operation: 'upload',
  transfers: ['basic'],
  objects: objects.map((_ref12) => {
    let {
      sha
    } = _ref12,
        rest = _objectWithoutProperties(_ref12, ["sha"]);
...
const getResourceUploadURLs = async (_ref13, objects) => {
  let {
    rootURL,
    makeAuthorizedRequest
  } = _ref13;
  const response = await makeAuthorizedRequest({
    url: "".concat(rootURL, "/objects/batch"),
    method: 'POST',
    headers: defaultContentHeaders,
...
const uploadBlob = (uploadURL, blob) => fetch(uploadURL, {
  method: 'PUT',
  body: blob
});

const uploadResource = async (clientConfig, _ref14, resource) => {
  let {
    sha,
    size
  } = _ref14;
