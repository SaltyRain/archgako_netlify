...
class GitGateway {
  ...
  async loadEntryMediaFiles(branch, files) {
    ...
    const mediaFiles = await Promise.all(files.map(async file => {
      if (client.matchPath(file.path)) {
        ...
        const largeMediaDisplayURLs = await this.getLargeMediaDisplayURLs([_objectSpread({}, file, {
          id
        })]);
  ...
  getMedia() {
    ...
    return Promise.all([this.backend.getMedia(mediaFolder), this.getLargeMediaClient()]).then(async (_ref) => {
  ...
  _getLargeMediaClient() {
    const netlifyLargeMediaEnabledPromise = this.api.readFile('.lfsconfig').then(config => _ini.default.decode(config)).then((_ref4) => {
      let {
        lfs: {
          url
        }
    ...
    const lfsPatternsPromise = this.api.readFile('.gitattributes').then(attributes => (0, _netlifyLfsClient.getLargeMediaPatternsFromGitAttributesFile)(attributes)).then(patterns => ({
      err: null,
      patterns
    })).catch(err => {
  ...
  async getLargeMediaDisplayURLs(mediaFiles) {
    const client = await this.getLargeMediaClient();
    const filesPromise = (0, _netlifyCmsLibUtil.entriesByFiles)(mediaFiles, this.api.readFile.bind(this.api), 'Git-Gateway');
    return filesPromise.then(items => items.map((_ref6) => {
      let {
        file: {
          id,
          path
    ...
    })).then(_unzip2.default).then((_ref7) => {
      let [idMaps, files] = _ref7;
      return Promise.all([idMaps, client.getResourceDownloadURLArgs(files).then(r => (0, _fromPairs2.default)(r))]);
    }).then((_ref8) => {
      let [idMaps, resourceMap] = _ref8;
      return idMaps.map((_ref9) => {
        let {
          pointerId,
          resourceId
        } = _ref9;
