...
class Backend {
  ...
  async entryExist(collection, path, slug, useWorkflow) {
    ...
    const publishedEntry = await this.implementation.getEntry(path).then((_ref) => {
      let {
        data
      } = _ref;
      return data;
  ...
  async search(collections, searchTerm) {
    ...
    const hits = entries.filter((_ref2) => {
      let {
        score
      } = _ref2;
      return score > 5;
  ...
  traverseCursor(cursor, action) {
    ...
    return this.implementation.traverseCursor(unwrappedCursor, action).then(async (_ref3) => {
      let {
        entries,
        cursor: newCursor
      } = _ref3;
  ...
  async getEntry(state, collection, slug) {
    ...
    if (collection.has('media_folder') && !integration) {
      entry.mediaFiles = await this.implementation.getMedia((0, _entries.selectMediaFolder)(state.config, collection, (0, _immutable.fromJS)(entryWithFormat)));
    } else {
  ...
  unpublishedEntries(collections) {
    return this.implementation.unpublishedEntries().then(entries => entries.map(loadedEntry => {
      ...
      const entry = (0, _Entry.createEntry)(collectionName, loadedEntry.slug, loadedEntry.file.path, {
        ...
        label: (0, _collections.selectFileEntryLabel)(collection, loadedEntry.slug)
      });
  ...
  async persistEntry(_ref4) {
    ...
    if (newEntry) {
      ...
      assetProxies.map(asset => {
        ...
        const newPath = (0, _entries.selectMediaFilePath)(config, collection, entryDraft.get('entry').set('path', path), oldPath);
        asset.path = newPath;
      });
    } else {
